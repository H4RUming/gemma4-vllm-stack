...
| MTP=5、シングルリクエスト           | 103.2 tok/s (σ ~3)   | **2.45x**      | 初回 5 回ベンチマーク                 |
| MTP=5、3 並列 (リクエストあたり)    | ~99 tok/s            | 2.35x          | シングルの 96% — ほぼ線形              |
| MTP=5、3 並列 (集計)                | **294 tok/s**        | **7.0x**       | wall time 8.2s vs 7.7s シングル (+6%) |
| MTP=5、sustained (実トラフィック)   | 100–125 tok/s        | 2.4–3.0x       | per-position 受理率 0.99/0.98/0.96/0.94/0.89; 平均 draft 受理率 **94–97%** (2026-05-23 再計測) |
...
- モデル重み: **約 33 GB** (BCCard FP8-Dynamic、BF16 + F8_E4M3 混在)。重み + KV プール + CUDA Graph + ドラフトモデルを含む GPU 占有量は約 89 GB。
- KV キャッシュプール: **495,400 トークン** (`--kv-cache-dtype fp8`、`block_size=16`、`num_gpu_blocks=35,447`; 2026-05-23 の `--max-num-batched-tokens` 変更後に計測 — [ADR-007](docs/adr/007-max-num-batched-tokens-32768.md) を参照)
- リクエストあたりの最大コンテキスト: **262,144** (256K、`--max-model-len`)
- 256K コンテキスト時の保証同時実行数: **~1.89x** (`495,400 / 262,144`)。プールサイズは設定依存のため、フラグを変更した場合は再計算が必要。
- BF16 KV と比較したデコード ITL の改善: 約 32% (vLLM が公開している Gemma 4 の数値に基づく)

稼働中のサービスで KV プールを直接確認:
...
1. **FP8 KV キャッシュ、TurboQuant ではない。** Gemma 4 で TurboQuant は 2 つの独立した問題により使えない。第一に、head dimension が不均一 (256 local、512 global) のため `TRITON_ATTN` バックエンドが選択されるが、このバックエンドは `kv_cache_dtype` を実装していない。第二に、vLLM PR #40534 が追加した `use_bidirectional_attention='vision'` が full-attention レイヤーにまで `use_mm_prefix=True` を伝播する。標準 FP8 e4m3 によって測定可能な精度劣化なしにデコード ITL を約 32% 改善できる。現在のプールサイズは 495,400 トークン / 256K で ~1.89x の保証同時実行であり、これは [ADR-007](docs/adr/007-max-num-batched-tokens-32768.md) のチャンクサイズ選択によって決まる。[ADR-001](docs/adr/001-fp8-kv-over-turboquant.md) 参照。
...
3. **`num_speculative_tokens=5`、モデルカード推奨の 4 ではない。** 測定結果: MTP=4 は 95–98 tok/s、MTP=5 は 103.2 tok/s。5% の改善が無料で手に入る。vLLM の >1 警告は一般的な注意喚起にすぎず、このハードウェアやドラフト構成に特化したものではない。初回 5 回測定の acceptance rate は 78%、2026-05-23 の実トラフィック再計測では sustained 94–97%。[ADR-003](docs/adr/003-mtp-num-spec-tokens-5.md) 参照。
...
7. **`--max-num-batched-tokens 32768` (明示的な prefill チャンク)。** ステップあたりのトークン予算を制限し、prefill バースト時の activation memory を予測可能にする。代償として KV プールが約 14% 減少する (576,912 → 495,400 トークン)。プールサイズは固定の約束ではなく、計測値として扱う。[ADR-007](docs/adr/007-max-num-batched-tokens-32768.md) 参照。

---

## デプロイレイアウト (本番ホスト)

上のリポジトリツリーはレビュー時の見やすさを意図した整理済みの形である。本番ホストのワーキングツリーは **フラット** で、systemd unit ファイル、wrapper スクリプト、proxy clone はすべて `${LLM_OPS_DIR}` (デフォルト `/home/haru/Desktop/LLM-OPS`) の最上位に置く。各 `*.service` unit の `WorkingDirectory` がそのパスで宣言されており、`ExecStart=/home/haru/Desktop/LLM-OPS/run_vllm.sh` がそこで resolve されるためである。

```
/home/haru/Desktop/LLM-OPS/        (= 2 つの .service unit の WorkingDirectory)
├── run_vllm.sh                    (このリポジトリの systemd/ からコピー)
├── run_proxy.sh                   (このリポジトリの systemd/ からコピー)
├── vllm-gemma4.service            (/etc/systemd/system/ にもインストール)
├── vllm-proxy.service             (/etc/systemd/system/ にもインストール)
├── proxy/                         (github.com/H4RUming/gemma4-vllm-proxy の clone)
├── logs/                          (ランタイム、gitignored)
├── hf-cache/                      (HF_HOME)
├── .venv/                         (vLLM サービング)
├── .venv-proxy/                   (FastAPI プロキシ)
└── .venv-quant/                   (llmcompressor; .venv には絶対にインストールしない)
```

`docs/deployment.md §9` の配置手順では、このリポジトリの `systemd/run_vllm.sh` と `systemd/run_proxy.sh` を `${LLM_OPS_DIR}/` へ、`.service` ファイルを `/etc/systemd/system/` へコピーする。フラットレイアウトは意図的なものであり、unit に埋め込まれた絶対パスと一致することで `daemon-reload → restart` が self-contained に成立する。
...
日付付きのトラブルシューティングノートは [`docs/investigations/`](docs/investigations/) に置いている。特定インシデントの診断過程 — 観測された現象、除外した仮説、適用した変更 — をそのまま記録するための場所であり、決定を記録する ADR や測定を記録する benchmarks とは役割が異なる。

- [2026-05-23 — prefill パフォーマンス調査](docs/investigations/2026-05-23-prefill-investigation.md)。2026-05-20 前後に p50 入力トークンが 11 倍 (約 37K) に増加したことで発生した TPS 低下 (~90 → ~25 tok/s) の原因を追跡している。ボトルネックが prefill にあることを確定させ、[ADR-007](docs/adr/007-max-num-batched-tokens-32768.md) に記録されている `--max-num-batched-tokens 32768` の設定を確定した。次に最大の余地はクライアント側のキャッシュキー安定化であると整理している。
