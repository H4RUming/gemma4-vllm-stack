...
| プロキシ           | 別 venv `.venv-proxy` 上の FastAPI (torch 無し) | ロギング、サンプリングデフォルト、thinking パラメータの正規化を担当。[ADR-005](docs/adr/005-logging-proxy-separation.md) 参照。 |
...
5. **自作 FastAPI プロキシを別 venv で運用。** LiteLLM/Langfuse/Helicone はいずれも ~10 名規模のデプロイに必要なものを超える依存を引き込み、リクエストログのスキーマを覆い隠す。約 300 行、自前のログスキーマ、外部ストレージ無し、数秒で起動、約 400 MB RAM、torch 無し。`.venv-proxy` に配置することで vLLM の依存変更がプロキシを壊さない。[ADR-005](docs/adr/005-logging-proxy-separation.md) 参照。
